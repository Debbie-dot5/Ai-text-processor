...
const LanguageTranslator = () => {
  ...
  const [targetLanguage, setTargetLanguage] = useState("en"); // Default: French
  ...
  useEffect(() => {
    const initializeAI = async () => {
      ...
      if (langCapabilities.capabilities === "no" || transCapabilities.capabilities === "no") {
      ...
      try {
        ...
        if (langCapabilities.capabilities === "readily") {
        ...
        let translatorInstance = await self.ai.translator.create({
            ...
            targetLanguage: "en",   // Default translation target
          });
  ...
  useEffect(() => {
    ...
    const translateText = async () => {
      try {
        const isSupported = await translator.capabilities();
        if (!isSupported.languagePairAvailable(detectedLanguage, targetLanguage)) {
          setTranslatedText("Translation not available for this language pair.");
          return;
        }

        const translation = await translator.translate(text, { sourceLanguage: detectedLanguage, targetLanguage });
